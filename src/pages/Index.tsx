      ...
      >
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          <button
            ...
            className="font-oswald text-xl font-bold gradient-text-gold tracking-wider"
          >
            💎 MINEWIN
          </button>
          ...
          {navItems.slice(0, 5).map(item => (
            <button
              ...
              className={`relative flex-1 flex flex-col items-center gap-1 py-3 transition-all ${
                page === item.id ? 'text-gold' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
              <Icon name={item.icon} fallback="Circle" size={20} />
              <span className="text-[10px] font-rubik">{item.label}</span>
              {page === item.id && (
                <div className="absolute bottom-0 w-6 h-0.5 rounded-full bg-gold" />
              )}
            </button>
          ))}
        </div>
